...
10. Highlight fenced code with a fixed Shiki theme and add trusted copy controls.
...
Syntax highlighting runs after sanitization. The only post-sanitization markup is generated by
Glyphweave itself: Shiki token spans with fixed theme colors and a button without inline event
