...
describe('PDF template', () => {
  ...
  it('defines editorial page furniture and block-specific rhythm', async () => {
    ...
    expect(template).toContain('counter(heading).display(it.numbering)')
    expect(template).toContain('let line-leading = 0.72em')
    expect(template).toContain('let block-spacing = 0.9em')
    expect(template).toContain('let heading-spacing = 0.95em')
